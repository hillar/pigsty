...
function Spooler(options) {
  ...
  // as global.console trick did not work, moved from cli.sj
  // daemonize is removed in subprocess, so we need test 
  if (!process.stdout.isTTY) {
        var SysLogger = require('ain2'); // waiting https://github.com/bnoordhuis/node-unix-dgram/issues/4 to be closed
        self.logger = new SysLogger();
        self.logger.set({path:'/dev/log'});
  } else {
    self.logger = console;
  }
    ...
    var setup = function(config) {
      ...
      try {
        ...
        plugin.on('error', function(error) {
          ...
          // more grok friendly msg here
          self.logger.error("Received error from plugin " + plugin_name + ": ", error);
        })
      } catch (err) {
        debug("plugin load failure: ", err.stack)
        self.logger.error("[!] There was a problem installing the output plugin: " + plugin_name);
        self.logger.error("[!] Error was:", err);
        self.logger.error("[!] Do you have the plugin installed? Try doing: npm install " + plugin_name + " -g");
        self.logger.error("[!] Have a special day. ♥")
        process.exit(1);
      };
...
Spooler.prototype._monitor_file = function(file) {
  ...
  if (file) {
    ...
    try {
      ...
      self.file = null;
      self.logger.log("[!] Error starting up file monitor, skipping:", file.filepath, "Got error:", err.message);
      debug("Error was:", err.stack);
      self.logger.log("[!] Please verify you have permissions to read from:", file.filepath);
      self.next_file();
    }
  }
